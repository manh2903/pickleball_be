  ...
  try {
    ...
    if (key === 'default_commission_rate') {
      ...
      const emailTasks = owners.map(async (owner) => {
        return sendEmail({
          to: owner.email,
          subject: '🔔 Thông báo thay đổi Phí hoa hồng Nền tảng - Pickleball Hub',
          html: `
            <div style="font-family: Arial, sans-serif; padding: 20px; line-height: 1.6; border: 1px solid #eee; border-radius: 10px;">
              <h2 style="color: #0ea5e9;">Pickleball Hub - Thông báo quan trọng</h2>
              <p>Xin chào <strong>${owner.name}</strong>,</p>
              <p>Chúng tôi xin thông báo về việc cập nhật chính sách phí hoa hồng trên nền tảng Pickleball Hub.</p>
              ...
              <p>Trân trọng,<br>Đội ngũ Pickleball Hub</p>
            </div>
          `
        });
      });
